...
const cancelEnter = document.getElementById('cancel-enter')
...
const buttonsEnter = enterPlay => {
    enterPlay()
    renderEnterButtons()
}

function renderEnterButtons() {

    if (buttonsEnt) {
        textArea.className = 'hidden'
        cancelEnter.className = 'hidden'
        showGame.className = 'hidden'
    } else {
        textArea.removeAttribute('class')
        cancelEnter.removeAttribute('class')
        showGame.removeAttribute('class')
    }
...
const initialization = () => {
    ...
    enter.addEventListener('click', () => buttonsEnter(enterButtonOnClick))
    showGame.addEventListener('click', () => showArrangementGame())
...
const validateArrangements = textArrangements => {
    if (textArrangements !== null && textArrangements !== "") {
        for (let i = 0; i < textArrangements.length; i++) {
            let term = textArrangements[i].trim()
